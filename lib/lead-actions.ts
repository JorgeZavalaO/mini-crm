...
export async function createLeadAction(input: unknown) {
  ...
  const data = {
    ...
    constitutionYear: payload.constitutionYear ?? null,
    employeeCount: payload.employeeCount ?? null,
    importOperationCount: payload.importOperationCount ?? null,
    exportOperationCount: payload.exportOperationCount ?? null,
...
export async function exportLeadsAction(tenantSlug: string): Promise<{
  ...
  const dataRows = leads.map((lead, leadIndex) => {
    ...
    return [
      ...
      lead.district ?? '',
      lead.address ?? '',
      lead.constitutionYear?.toString() ?? '',
      lead.employeeCount?.toString() ?? '',
      lead.importOperationCount?.toString() ?? '',
      lead.exportOperationCount?.toString() ?? '',
      lead.industry ?? '',
      lead.source ?? '',
      lead.gerente ?? '',
